        ...
        try:
            ...
            async with session.get(img_url) as response:
                if response.status == 200:
                    ...
                    print(
                        f"Failed to download image from {img_url}. Status: {response.status}"
                    )
...
async def scrape_google_images(
    search_query="macbook m3", max_images=None, timeout_duration=10
):
    """
        ...
        async with ClientSession(
            ...
            for idx, image_element in enumerate(image_elements):
                if max_images is not None and images_downloaded >= max_images:
                    print(
                        f"Reached max image limit of {max_images}. Stopping download."
                    )
                    break
                try:
                    ...
                    img_tag = await page.query_selector(
                    ...
                    file_extension = (
                        os.path.splitext(urlparse(img_url).path)[1] or ".png"
                    )
                    file_path = os.path.join(
                        download_folder, f"image_{idx + 1}{file_extension}"
                    )
                    ...
                    source_url = await page.query_selector(
                        '(//div[@jsname="figiqf"]/a[@class="YsLeY"])[2]'
                    )
                    source_url = (
                        await source_url.get_attribute("href") if source_url else "N/A"
                    )
...
asyncio.run(
    scrape_google_images(search_query=query, max_images=50, timeout_duration=10)
)
